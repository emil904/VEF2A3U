...
class Upload{
	...
	protected function checkFile($file){ #athugar hvort það sé í lagi með skrá
		...
		if ($file['error'] != 0) {
			...
			//ekki checka ef engin skrá uploaduð
			if ($file['error'] == 4) {
				return false;
			} else{
				$accept = false;
		...
		if (!$this->checkSize($file)) {
			#returnar false ef file er of stór
			$accept = false;
		}
	...
	protected function moveFile($file){ #Færir skrár úr TMP yfir í upload_test folder
		...
		if ($success) {
			...
			#echo 'File type is ' . $file['type'] . ' and filesize is: ' . $file['size'];
			if (!is_null($this->newName)) {
				$result .= ', and was renamed to ' . $this->newName;

			}
			$result .= "\n File type is " . $file['type'] . " and filesize is: " . number_format($file['size']/1024, 1) . "KB";
	...
	protected function checkName($file){
		$this->newName = null;
		$nospaces = preg_replace('/[^a-zA-Z0-9_.]/', '', $file['name']);
	...
	public function upload($renameDuplicates = true){ #Upload fall
